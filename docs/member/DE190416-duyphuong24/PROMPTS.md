...
| Công cụ AI | Antigravity |
| Mục đích | Xác thực thiết kế trạng thái Draft và Scope của bài tập khi xóa Contest |
| Phần việc liên quan | Coding / Database / Design / UI-UX |
| Mức độ sử dụng | Hỏi tối ưu / Hỏi kiểm chứng |
...
```text
Sinh viên nhận thấy phương thức hardDeleteAdminContest đang thay đổi cả `problemScope = PRACTICE` lẫn `isPublic = false` cho các bài tập thuộc kỳ thi bị xóa. Sinh viên muốn làm rõ vai trò của trường `isPublic` (tương ứng với Draft/Publish trên UI) và tính cần thiết của việc thay đổi `problemScope`.
```

#### 5.3. Kết quả AI trả về

```text
AI giải thích rằng `isPublic = false` chính xác là để đưa bài tập về chế độ riêng tư (Draft) trong tab "Repository & Drafts" của Admin. Tuy nhiên, việc reset `problemScope` từ `CONTEST` về `PRACTICE` là bắt buộc vì nếu không, sau này khi admin publish bài tập đó lên, nó sẽ hiển thị sai ở tab "Contest Problems" mặc dù cuộc thi gốc đã không còn tồn tại.
```

#### 5.4. Kết quả đã áp dụng vào bài

```text
Giữ nguyên logic cập nhật đồng thời cả hai thuộc tính `isPublic` và `problemScope` tại backend để đảm bảo an toàn nghiệp vụ và tính nhất quán dữ liệu.
```

#### 5.5. Phần sinh viên/nhóm đã chỉnh sửa hoặc cải tiến

```text
Đồng bộ hóa logic này vào cả tính năng xóa đơn lẻ bài tập khỏi cuộc thi (`removeProblemFromContest`), giúp đồng nhất trải nghiệm quản trị hệ thống.
```
...
| File liên quan | backend/src/main/java/com/swp391/coding_platform/service/contest/ContestService.java |
| Screenshot | |
| Kết quả chạy/test | Đã chạy thử nghiệm xóa/gỡ bài tập, bài tập trả về trạng thái Draft PRACTICE thành công. |
| Ghi chú khác | |

#### 5.8. Ghi chú thêm

```text
Prompt dạng hỏi phản biện (Socratic/Reflective prompt) giúp dev hiểu rõ hơn về các tác động biên (side effects) của việc thiết kế database đến giao diện frontend.
```
